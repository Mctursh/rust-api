...
pub type DbOpenResult = DbResult<(FileHeader, File)>;
...
impl Database {
    ...
    pub fn read_page(&mut self , page_id: u32) -> DbResult<[u8; PAGE_SIZE as usize]> {
        ...
        if let Some(page) = self.buffer_pool.find_page(page_id)? {
            println!("Read page with id: {:?}", page.page_id);
    ...
    // fn open_database(self, path: &Path) -> DbOpenResult {
    //     // let mut file = OpenOptions::new().read(true).write(true).open(path)?;

    //     let mut file_header = [0u8; PAGE_SIZE as usize];
    //     &self.file.read_exact(&mut file_header)?;
